...
"""Configuration for serverless hot GPU → cold shadow CPU KV migration."""
...
class ShadowMigrationConfig:
    ...
    enable_shadow_migration: bool = False
    ...
    shadow_tkcth_ipc_prefix: str | None = None
    """Unix socket path prefix for the hot-side TKCTH listener (token stream back from
    shadow). A unique path is formed by appending a random suffix per migration batch,
    analogous to ``--shadow-kvhtc-ipc-prefix`` on the shadow process.
    """
