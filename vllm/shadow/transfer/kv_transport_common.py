...
"""Shared building blocks for shadow KV transfers (memfd CPU tensors, dtype wire
strings, per-layer KV byte layout).

Framed UDS send/recv and ACK live in :mod:`vllm.shadow.transfer.uds`. JSON handoffs
live in ``kvhts_protocol`` / ``kvstc_protocol``. Per-layer memfds use
:data:`LAYER_MEMFD_INLINE_PAYLOAD` on the UDS primary buffer; **layer order** is the
send/recv sequence (``0 .. num_layers-1``), not a per-layer wire struct.

This module intentionally avoids importing vLLM. It uses ``torch`` for dtypes and
tensor views over ``mmap`` regions.
"""
...
# UDS ``sendmsg`` requires a non-empty primary buffer with ``SCM_RIGHTS``.
LAYER_MEMFD_INLINE_PAYLOAD: Final[bytes] = b"\x01"
