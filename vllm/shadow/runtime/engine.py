...
class _KvhtsSession:
    ...
    def run(self) -> _KvhtsSessionResults | None:
        try:
    ...
    def _run_migration_session(self) -> _KvhtsSessionResults:
        assert self._kvhts_receiver is not None
        receiver = self._kvhts_receiver
        ...
        logger.info(
            "kvhts migration done: migration_id=%s requests=%d "
            "layers=%s num_blocks=%s bytes≈%s time_s=%.3f",
...
class _KvstcSession:
    ...
    def run(self) -> None:
        try:
            ...
            logger.exception("kvstc session failed: %s", e)
            raise e
        finally:
            if self._kvstc_sender is not None:
                with suppress(OSError):
                    self._kvstc_sender.close()
    ...
    def _run_migration_session(self) -> None:
        ...
        logger.info(
            "Running kvstc migration session: migration_id=%s kvstc_ipc_path=%s",
...
class ShadowEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self._kvhts_session = _KvhtsSession(
    ...
    def run(self) -> None:
        try:
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                kvhts_future = executor.submit(self._kvhts_session.run)
                model_future = executor.submit(self._model_loader.run)

            if self._stop_event.is_set():
                return

            kvhts_results = cast(_KvhtsSessionResults, kvhts_future.result())
            handoff, kv_state = (
                kvhts_results.handoff,
                kvhts_results.kv_state,
            )
