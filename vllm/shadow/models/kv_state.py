...
structures needed by the cold shadow CPU path to ingest migrated KV blocks and
...
from vllm.shadow.transfer.kvhtc_protocol import torch_dtype_from_str
...
class ShadowKvState:
    ...
    This type deliberately avoids engine/scheduler imports so the cold shadow
    ...
    def prepare_request(
        self, request_id: str
    ) -> tuple[list[int], int, ShadowSamplingParams] | None:
        """Prepare a request for forward pass.

        Returns the uncomputed token ids, the number of computed tokens,
        and the sampling parameters.
        Returns None if the request has no uncomputed tokens.
        """
        ...
        for pos in range(nc, nc + len(tids)):
            ...
            if not successed:
                return None
        return tids, nc, req.sampling_params
