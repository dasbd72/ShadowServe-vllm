...
class ShadowKvState:
    ...
    def __init__(
        ...
        self.expected_dtype = torch_dtype_from_str(dtype)
        self.expected_shape = (
            2,
            num_blocks,
            num_kv_heads,
        ...
        self.layers: list[MemfdTensor | None] = [None] * num_layers
    ...
    def register_layer(
        ...
        if tuple(t.shape) != self.expected_shape:
            raise ValueError(
                f"layer KV shape {tuple(t.shape)} != expected {self.expected_shape}"
            )
        if t.dtype != self.expected_dtype:
            raise ValueError(
                f"layer KV dtype {t.dtype} != expected {self.expected_dtype}"
            )
        self.layers[layer_idx] = tensor
    ...
    def add_request(
        ...
        for bid in block_table:
            self.free_blocks.discard(bid)

    def remove_request(self, request_id: str) -> None:
        """Remove a request and return its physical blocks to the free pool."""
    ...
    def _ensure_block_for_logical(
        ...
        while logical_block_idx >= len(block_table):
            if not self.free_blocks:
                return False
            block_table.append(self.free_blocks.pop())
        return True
    ...
    def free_request(self, request_id: str) -> None:
        ...
        self.free_blocks.update(req.block_table)
    ...
    def build_attention_batch(
        ...
        n = len(request_ids)
        max_blocks = max(len(self.requests[rid].block_table) for rid in request_ids)
    ...
    def _layer_kv_caches(self) -> list[torch.Tensor]:
        ...
        **K** uses the packed tile layout from ``gather_kv_blocks`` /
        ...
        """
        kv_caches: list[torch.Tensor] = []
        for i, tensor in enumerate(self.layers):
            if tensor is None:
