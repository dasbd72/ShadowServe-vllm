...
def _load_qkv_weights(w: dict[str, torch.Tensor], layer_idx: int) -> torch.Tensor:
    fused = f"layers.{layer_idx}.self_attn.qkv_proj.weight"
    if fused in w:
        return w.pop(fused)
...
def _load_qkv_bias(w: dict[str, torch.Tensor], layer_idx: int) -> torch.Tensor | None:
    ...
    fused = f"layers.{layer_idx}.self_attn.qkv_proj.bias"
    if fused in w:
        return w.pop(fused)
...
def _load_gate_up_weights(w: dict[str, torch.Tensor], layer_idx: int) -> torch.Tensor:
    fused = f"layers.{layer_idx}.mlp.gate_up_proj.weight"
    if fused in w:
        return w.pop(fused)
...
def _load_gate_up_bias(
    ...
    fused = f"layers.{layer_idx}.mlp.gate_up_proj.bias"
    if fused in w:
        return w.pop(fused)
...
def build_llama_for_causal_lm(
    ...
    *,
    ...
    for i in range(L):
        ...
        o_bias = w.get(f"layers.{i}.self_attn.o_proj.bias")
        ...
        down_b = w.get(f"layers.{i}.mlp.down_proj.bias")
