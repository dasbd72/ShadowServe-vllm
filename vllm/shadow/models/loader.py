...
logger = logging.getLogger("vllm.shadow.models.loader")

__all__ = ("ShadowModelLoader",)
...
class ShadowModelLoader:
    ...
    def __init__(self, model_config: ShadowModelConfig) -> None:
        self._config = model_config

        self.model_dir = self._resolve_model_dir()
        self.hf_config = load_shadow_hf_config(self.model_dir)

    # ── Model directory resolution ────────────────────────────────────

    def _resolve_model_dir(self) -> str:
        """Resolve ``model_config.model`` to a local directory.
