...
class EngineArgs:
    ...
    shadow_kvstc_ipc_prefix: str | None = get_field(
        ShadowMigrationConfig, "shadow_kvstc_ipc_prefix"
    )
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        shadow_group.add_argument(
            ...
            **shadow_kwargs["shadow_kvstc_ipc_prefix"],
        )
    ...
    def create_engine_config(
        ...
        shadow_migration_config = ShadowMigrationConfig(
            ...
            shadow_kvstc_ipc_prefix=self.shadow_kvstc_ipc_prefix,
        )
        if "VLLM_SHADOW_SENDER_ENABLED" in os.environ:
            shadow_migration_config = replace(
                shadow_migration_config,
                shadow_sender_enabled=envs.VLLM_SHADOW_SENDER_ENABLED,
            )
        ...
        if "VLLM_SHADOW_TKSTH_IPC_PREFIX" in os.environ:
            raw = os.environ["VLLM_SHADOW_TKSTH_IPC_PREFIX"].strip()
            shadow_migration_config = replace(
                shadow_migration_config,
                shadow_tksth_ipc_prefix=raw or None,
            )
        if "VLLM_SHADOW_RECEIVER_ENABLED" in os.environ:
            shadow_migration_config = replace(
                shadow_migration_config,
                shadow_receiver_enabled=envs.VLLM_SHADOW_RECEIVER_ENABLED,
            )
        if "VLLM_SHADOW_KVSTC_IPC_PREFIX" in os.environ:
            raw = os.environ["VLLM_SHADOW_KVSTC_IPC_PREFIX"].strip()
            shadow_migration_config = replace(
                shadow_migration_config,
                shadow_kvstc_ipc_prefix=raw or None,
            )
        ...
        if shadow_migration_config.shadow_receiver_enabled:
            kv_pfx = shadow_migration_config.shadow_kvstc_ipc_prefix
            if kv_pfx is None or not str(kv_pfx).strip():
                raise ValueError(
                    "shadow_receiver_enabled requires a non-empty "
                    "shadow_kvstc_ipc_prefix "
