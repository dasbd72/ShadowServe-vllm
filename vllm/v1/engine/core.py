...
class EngineCore:
    ...
    def __init__(
        ...
        self._completed_tksth_sessions: list[ShadowTksthSession] = []
        self._shadow_kvstc_sessions: list[ShadowKvstcSession] = []
        self._completed_kvstc_sessions: list[ShadowKvstcSession] = []
    ...
    def shadow_migration_migrate(
        ...
        tk_pfx = smc.shadow_tksth_ipc_prefix
        if tk_pfx is None or not str(tk_pfx).strip():
            raise RuntimeError(
                "shadow migration requires shadow_tksth_ipc_prefix "
                "(--shadow-tksth-ipc-prefix / VLLM_SHADOW_TKSTH_IPC_PREFIX)"
            )
        tksth_ipc_path = f"{str(tk_pfx).strip()}-{secrets.token_hex(8)}"
    ...
    def shadow_migration_get_requests(
    ...
    def _drain_shadow_events(self) -> list[tuple[int, EngineCoreOutputs]]:
        ...
        self._completed_tksth_sessions.extend(
            [s for s in self._shadow_tksth_sessions if s.is_done]
        )
    ...
    def shadow_migration_recv(
        ...
        session = ShadowKvstcSession(kvstc_ipc_path=kvstc_ipc_path)
        self._shadow_kvstc_sessions.append(session)

    def shadow_migration_completed(self) -> dict[str, list[int]]:
        return {
            "completed_tksth_sessions": [
                s.migration_id for s in self._completed_tksth_sessions
            ],
            "completed_kvstc_sessions": [
                s.handoff.migration_id for s in self._completed_kvstc_sessions
            ],
        }
    ...
    def _drain_shadow_kvstc_events(self) -> None:
        ...
        self._completed_kvstc_sessions.extend(
            [s for s in self._shadow_kvstc_sessions if s.is_done]
        )
