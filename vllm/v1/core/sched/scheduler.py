...
class Scheduler(SchedulerInterface):
    ...
    def detach_requests_for_shadow_migration(
        ...
        for rid in request_ids:
            ...
            if request is None:
                raise ValueError(f"kvhtc migration: unknown request_id {rid!r}")
            if request.is_finished():
                logger.warning("kvhtc migration: request %r already finished", rid)
                continue
            if request.status != RequestStatus.RUNNING:
                logger.warning(
                    "kvhtc migration: request %r must be RUNNING (got %s)",
                    rid,
                    request.status,
                )
                continue
            if rid not in running_set:
                raise ValueError(
                    f"kvhtc migration: request {rid!r} is RUNNING but "
    ...
    def promote_migrated_requests_to_shadow_running(
        ...
        shadow until the shadow TKCTH session ends.
        """
        for rid in request_ids:
            request = self.requests.get(rid)
            if request is None:
