    ...
    def shadow_kvhts_migrate(
        ...
        for i, req in enumerate(requests):
            ...
            handoff_requests.append(
                KvhtsRequest(
                    request_id=req_id,
                    client_index=int(req.client_index),
                    prompt_token_ids=(
        ...
        logger.info(
            "kvhts migration batch done: kvhts_migration_id=%s kvhts_requests=%d "
            "kvhts_layers=%d kvhts_shadow_blocks=%d kvhts_bytes≈%d kvhts_time_s=%.3f "
            "kvhts_tksth_ipc_path=%s",
            migration_id,
            batch_size,
            num_layers,
            shadow_num_blocks,
