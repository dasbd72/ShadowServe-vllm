...
#
# - Without --enable-shadow-migration-cold: behaves like `vllm serve ...`
# - With --enable-shadow-migration-cold: also starts a shadow CPU process
...
def _split_args(argv: Sequence[str]) -> tuple[bool, str, list[str]]:
    """Return (enable_shadow_cold, shadow_kvhtc_ipc_prefix, main_args)."""
    enable_shadow_cold = False

    # First pass: strip cold-only flag.
    args: list[str] = []
    for arg in argv:
        if arg == "--enable-shadow-migration-cold":
            enable_shadow_cold = True
        else:
            args.append(arg)

    # Second pass: extract and remove --shadow-kvhtc-ipc-prefix pair.
    shadow_kvhtc_ipc_prefix = ""
    main_args: list[str] = []
    i = 0
    while i < len(args):
        if args[i] == "--shadow-kvhtc-ipc-prefix":
            if i + 1 >= len(args):
                print(
                    "[entrypoint] ERROR: --shadow-kvhtc-ipc-prefix requires a value",
                    file=sys.stderr,
                    flush=True,
                )
                raise SystemExit(2)
...
def main() -> int:
    ...
    enable_shadow_cold, shadow_kvhtc_ipc_prefix, main_args = _split_args(sys.argv[1:])
    ...
    if enable_shadow_cold:
        shadow_cmd = ["python3", "-m", "vllm.shadow.cli.main"]
        if shadow_kvhtc_ipc_prefix:
