...
class ShadowBackend:
    ...
    async def run(self, num_sessions: int = 1) -> None:
        ...
        for session_id in range(num_sessions):
            migration_id = 2 + session_id
            kvhts_ipc_path = f"/tmp/kvhts-{session_id}.sock"
        ...
        await asyncio.gather(
            *[
                asyncio.to_thread(
                    self._hot_mock,
                    f"/tmp/tksth-{session_id}.sock",
                    2 + session_id,
                    f"/tmp/kvhts-{session_id}.sock",
                )
                for session_id in range(num_sessions)
            ]
        )
        elapsed = time.perf_counter() - t0
        logger.info("ShadowBackend.run %.3fs sessions=%d", elapsed, num_sessions)
    ...
    def _hot_mock(
        ...
        try:
            kvhts_transport.connect(kvhts_ipc_path)
...
class Context:
    ...
    async def run_concurrent(self) -> None:
        ...
        forward_ref = self.shadow_actor.run.remote(self.num_sessions)
        await self.start_cold()
    ...
    async def run_mem_bw(self) -> None:
        ...
        assert self.mem_bw_actor is not None
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="parallel cold GPU inits (and shadow KVHTS sessions in concurrent mode)",
    )
    return p.parse_args()
