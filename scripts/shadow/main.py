...
from proxy import StreamingChatCompletionProxy
...
def _hot_cold_and_shadow_env(
    ...
    hot_env["OMP_NUM_THREADS"] = str(shadow_num_threads)
    ...
    cold_env["OMP_NUM_THREADS"] = str(shadow_num_threads)

    shadow_env = os.environ.copy()
    shadow_env["CUDA_VISIBLE_DEVICES"] = ""
    shadow_env["VLLM_TARGET_DEVICE"] = "cpu"
    shadow_env["OMP_NUM_THREADS"] = str(shadow_num_threads)
...
class Executor:
    def __init__(
        ...
        *,
        ...
        self.shadow_num_threads = shadow_num_threads
        ...
        self.clients: list[StreamingChatCompletionProxy] = []
    ...
    def _start_streaming_client(
        self, i: int
    ) -> tuple[StreamingChatCompletionProxy, str]:
        c = StreamingChatCompletionProxy(
            self.hot_base_url,
            self.model,
            [{"role": "user", "content": f"This is request {i}. {self.user_message}"}],
    ...
    def _print_outputs(self) -> None:
        for oid, c in zip(self.openai_ids, self.clients, strict=True):
            ...
            c.record_pivot()
            text = c.output_text
            text_pivots = c.text_pivots
            token_pivots = c.token_pivots
            if text_pivots and text_pivots[0] != 0:
                text_pivots = [0] + text_pivots
            if token_pivots and token_pivots[0] != 0:
                token_pivots = [0] + token_pivots
            text_chunks = [
                text[text_pivots[i] : text_pivots[i + 1]]
                for i in range(len(text_pivots) - 2)
            ]
            text = " | ".join(text_chunks)
            logger.info("Request %s", oid)
            logger.info("Messages: %s", c.messages)
            logger.info("Output Text: %s", text)
            logger.info("Text Pivots: %s", text_pivots)
            logger.info("Token Pivots: %s", token_pivots)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        type=int,
        default=32,
        help="OMP_NUM_THREADS override for the shadow vLLM process.",
    )
    ap.add_argument(
    ...
    ap.add_argument("--user-message", default=Config.DEFAULT_SMOKE_USER_MESSAGE)
...
def main(argv: list[str] | None = None) -> int:
    ...
    executor = Executor(
        ...
        shadow_num_threads=args.shadow_num_threads,
